...
;; clojure.string is used instead of clojure.contrib.str-utils
...
(defn keys-for [keys separator values]
  (let [pk-value (string/join separator values)]
  ...
  (if-not (some #(= % key) string-attribs)
    (if-not (some #(= % key) list-attribs)
      (throw (RuntimeException.
	      (str "Attempt to use unknown key " key
		   " in redis-object of type " (redis-type :name))))))
  true)

(defn new-redis-type [name separator format primary-keys
		      string-attribs list-attribs]
  (fn redis-type [accessor & args]
    (condp = accessor
      ...
      :valid-key? (let [[key] args]
		    (check-key-validity key redis-type
					string-attribs list-attribs))
      ...
      :exists? (let [key-value (string/join separator args)
		     key-value (str key-value separator
				    (first primary-keys))]
		 (redis/exists key-value))
      :attrib-exists? (let [attrib-key (first args)
			    pk-value (string/join separator (rest args))]
			(redis/exists (str pk-value
					   separator attrib-key))))))
...
(defmacro def-redis-type [name & specs]
  (let [string-types (specs-for 'string-type specs)
    ...
    `(def ~name
	  (new-redis-type '~name ~separator ~format
			  '~pk-keys '~string-types '~list-types))))